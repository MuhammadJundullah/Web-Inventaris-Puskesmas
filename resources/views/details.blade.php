            ...
            <div class="mt-6 border-t border-gray-100">
                <dl class="divide-y divide-gray-100">
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        ...
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{$data['data']['nama_barang']}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Sumber dana</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{$data['data']['sumber_dana']}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Tanggal masuk</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{$data['data']['tanggal']}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Jumlah barang</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{$data['data']['jumlah']}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Editor</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{$data['data']['editor']}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Tanggal di input</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{\Carbon\Carbon::parse($data['data']['created_at'])->format('d-m-Y')}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        <dt class="font-medium leading-6 text-gray-900">Terakhir di ubah</dt>
                        <dd class="mt-1 leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{\Carbon\Carbon::parse($data['data']['updated_at'])->diffForHumans()}}</dd>
                    </div>
                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                        ...
                        <dd class="mt-2 text-gray-900 sm:col-span-2 sm:mt-0">
                            <ul role="list" class="divide-y divide-gray-100 rounded-md border border-gray-200">
                                <li class="flex items-center justify-between py-4 pl-4 pr-5 text-sm leading-6">
                                    <img class="rounded" src="{{ asset('storage/' . $data['data']['gambar']) }}" alt="gambar inventaris">
                                </li>
                            </ul>

                            <div class="mt-6 flex items-center justify-between gap-x-2">
                                <a href="/inventory/{{$data['year']}}" class="group inline-flex items-center gap-1 text-sm font-medium text-blue-600">
                                ...
                                <div class="flex gap-x-2 ml-auto">
                                    ...
                                    <span class="inline-flex overflow-hidden rounded-md border bg-white shadow-sm">
                                        <a href="/audit/edit/{{$data['year']}}/{{$data['data']['id']}}">
                                        ...
                                        <button onclick="openDeleteModal('{{ $data['year'] }}', {{$data['data']['id']}})" class="inline-block p-3 text-gray-700 hover:bg-gray-50 focus:relative" title="Delete Product">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
